...
import { BarChart2, Hash, Clipboard, Check } from "lucide-react";
...
export const LinkedInResultCard = ({ message }: LinkedInResultCardProps) => {
  const variations = message.resultData as any[]; // Humein pata hai yeh ek array hai
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(activePost.full_text);
    setCopied(true);
        ...
        <div className="flex items-center gap-4 text-sm">
          <div className="flex items-center gap-1.5 text-yellow-400">
            ...
            <span className="font-semibold">
              Engagement Score: {activePost.engagement_score}
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-neutral-400">
            <Hash className="w-4 h-4" />
            <span className="font-semibold">{activePost.word_count} words</span>
          </div>
        </div>

        {/* Full Text */}
        <p className="text-neutral-200 whitespace-pre-wrap text-sm leading-relaxed max-h-60 overflow-y-auto pr-2">
          {activePost.full_text}
        </p>
